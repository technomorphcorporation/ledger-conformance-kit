 ...
 *   <li><b>Every run is seeded.</b> The seed prints with the result and controls the workload:
 *       the amounts each invariant moves and the transaction ids it stamps, so a finding that
 *       names a transaction can be found again on a re-run. It does not control the
 *       interleaving — see {@link Harness} for exactly where the line falls, because a claim
 *       to reproduce a race would be a claim this kit cannot keep.</li>
...
public final class Invariants {
    ...
    /**
     * Where all non-determinism goes, seeded, so a run can be repeated.
     *
     * <p><b>What the seed controls, exactly.</b> The amounts an invariant moves and the
     * transaction ids it stamps. Two runs at the same seed post the same figures under the same
     * ids, so a finding that names a transaction can be found again on a re-run — which is the
     * whole reason a defect report carries one.
     *
     * <p><b>What it does not control.</b> The interleaving. Iteration counts and burst sizes are
     * fixed too: they are calibrated to open the window these defects live in, and varying them
     * would trade a known detection rate for variety. So the seed reproduces the <em>workload</em>
     * and not the schedule, and a concurrency finding may need more than one run to reappear.
     * Saying otherwise would be the sort of claim this kit exists to avoid making; a scheduler
     * that reproduces interleavings is a v2 item, in ROADMAP.
     *
     * <p>{@link #random()} is exposed for an invariant that needs a draw of its own. Do not hand
     * it to concurrent tasks: {@link Random} is thread-safe but the order of draws is not, so a
     * shared instance inside a {@link #burst} produces a different sequence each run and quietly
     * un-seeds the invariant. Draw before the burst, or derive a value from the task index.
     */
    public static final class Harness {
        ...
        /**
         * A transaction id that is the same on the next run at this seed.
         *
         * <p>Replaces {@code UUID.randomUUID()} at the call sites that matter. A finding reading
         * "first unbalanced transaction: 9f2c…" names something that will never exist again if
         * the id is random, which makes the number in the message useless for the one thing a
         * defect report is for.
         */
        public String txnId(String label) {
            return "lck-" + seed + "-" + label;
        }

        /** A transfer whose transaction id is derived from the seed and the key, not from a UUID. */
        public Transaction transfer(String src, String dst, long amountSubunits, String key) {
            return transfer(src, dst, amountSubunits, key, false);
        }

        public Transaction transfer(String src, String dst, long amountSubunits, String key,
                                    boolean allowOverdraft) {
            return new Transaction(key,
                    List.of(Leg.debit(src, amountSubunits), Leg.credit(dst, amountSubunits)),
                    txnId(key), allowOverdraft, Map.of());
        }

        /**
         * A seeded amount, in subunits, always a whole number of currency units and never zero.
         *
         * <p>Whole units because a finding reading "lost 3.00" is read faster than "lost 3.47",
         * and never zero because {@code Leg} refuses a zero amount — rounding the floor down
         * rather than up produced exactly that, and every concurrency invariant died with
         * "legs are unsigned" until the tests caught it.
         */
        public long amount(long minSubunits, long maxSubunits) {
            long minUnits = Math.max(1, (minSubunits + 99) / 100);
            long maxUnits = Math.max(minUnits, maxSubunits / 100);
            return (minUnits + rnd.nextInt((int) (maxUnits - minUnits + 1))) * 100;
        }
    ...
    public static final List<Invariant> REGISTRY = List.of(
        ...
        new Invariant("INV-01", "Every transaction is balanced per currency", Severity.BLOCKER,
            ...
            (led, h) -> {
                long amt = h.amount(1_000, 5_000);
                led.seed("acct:a", amt * 3);
                led.post(h.transfer("acct:a", "acct:b", amt, "t1"));

                // The ledger has to be offered a broken transaction, not merely watched handling
                // good ones. Without this the invariant only catches a ledger that corrupts a
                // balanced transaction on write, while its stated symptom — money created on
                // write — is the case that was never tested.
                int before = led.journal().size();
                PostResult unbalanced = led.post(new Transaction("unbalanced",
                        List.of(new Leg("acct:a", EntryType.DEBIT, amt, "USD"),
                                new Leg("acct:b", EntryType.CREDIT, amt - 100, "USD")),
                        h.txnId("unbalanced"), true, Map.of()));
                if (unbalanced.status() != PostStatus.REJECTED)
                    return Check.bad("a transaction debiting " + money(amt) + " and crediting "
                            + money(amt - 100) + " returned " + unbalanced.status() + " — "
                            + money(100) + " was created on write");
                int written = led.journal().size() - before;
                if (written != 0)
                    return Check.bad("an unbalanced transaction was refused but still wrote "
                ...
                return bad.isEmpty()
                        ? Check.ok(byTxn.size() + " transactions all net zero per currency, and an "
                                + "unbalanced one was refused")
                        : Check.bad(bad.size() + " unbalanced transaction(s) in the journal, first: "
                                + bad.get(0));
            }),
        ...
        new Invariant("INV-02", "The journal is append-only", Severity.BLOCKER,
            ...
            (led, h) -> {
                long amt = h.amount(500, 3_000);
                led.seed("acct:a", amt * 5);
                led.post(h.transfer("acct:a", "acct:b", amt, "t1"));
                List<String> before = fingerprint(led.journal());
                led.post(h.transfer("acct:b", "acct:a", amt, "t2"));
                led.post(h.transfer("acct:a", "acct:b", amt, "t3"));
        ...
        new Invariant("INV-03", "Hash chain is intact", Severity.MAJOR,
            ...
            (led, h) -> {
                long amt = h.amount(100, 2_000);
                led.seed("acct:a", amt * 10);
                for (int i = 0; i < 5; i++)
                    led.post(h.transfer("acct:a", "acct:b", amt, "t" + i));
                String prev = "GENESIS";
                for (JournalEntry e : led.journal()) {
        ...
        new Invariant("INV-04", "Duplicate submission is a no-op", Severity.BLOCKER,
            ...
            (led, h) -> {
                long amt = h.amount(500, 5_000);
                led.seed("acct:a", amt * 4);
                led.post(h.transfer("acct:a", "acct:b", amt, "dup-key"));
                int n1 = led.journal().size();
                PostResult r2 = led.post(h.transfer("acct:a", "acct:b", amt, "dup-key"));
                int n2 = led.journal().size();
                long bal = led.balance("acct:b");
                if (n2 != n1)
                    return Check.bad("replay wrote " + (n2 - n1) + " extra journal entries");
                if (bal != amt)
                    return Check.bad("balance " + money(bal) + " after replay, expected " + money(amt));
        ...
        new Invariant("INV-05", "Concurrent duplicates collapse to one", Severity.BLOCKER,
            ...
            (led, h) -> {
                final int n = 64;
                final long amt = h.amount(500, 5_000);
                led.seed("acct:a", amt * n);
                Tally t = new Tally();
                h.burst(n, i -> {
                    try {
                        t.record(led.post(h.transfer("acct:a", "acct:b", amt, "race-key")));
        ...
        new Invariant("INV-07", "Value is conserved under concurrent transfers", Severity.BLOCKER,
            ...
            (led, h) -> {
                ...
                final long amt = h.amount(50, 400);
                for (int i = 0; i < accounts; i++) led.seed("acct:" + i, amt * 20);
                long opening = 0;
                for (int i = 0; i < accounts; i++) opening += led.balance("acct:" + i);
                Tally t = new Tally();
                h.burst(moves, i -> {
                    try {
                        t.record(led.post(h.transfer("acct:" + (i % accounts),
                                "acct:" + ((i + 3) % accounts), amt, "mv-" + i, true)));
                    } catch (Exception e) { throw new RuntimeException(e); }
                });
        ...
        new Invariant("INV-08", "Balance equals the journal projection", Severity.BLOCKER,
            ...
            (led, h) -> {
                final long amt = h.amount(50, 500);
                led.seed("acct:a", amt * 200);
                h.burst(200, i -> {
                    try {
                        led.post(h.transfer("acct:a", "acct:b", amt, "proj-" + i, true));
                    } catch (Exception e) { throw new RuntimeException(e); }
                });
        ...
        new Invariant("INV-09", "Overdraft guard holds under a concurrent drain", Severity.BLOCKER,
            ...
            (led, h) -> {
                final int n = 20;
                final long amt = h.amount(500, 2_000), opening = amt * 10;
                led.seed("acct:a", opening);
                Tally t = new Tally();
                h.burst(n, i -> {
                    try {
                        t.record(led.post(h.transfer("acct:a", "acct:b", amt, "drain-" + i)));
                    } catch (Exception e) { throw new RuntimeException(e); }
                });
        ...
        new Invariant("INV-10", "No precision drift over many small movements", Severity.BLOCKER,
            ...
            (led, h) -> {
                // One subunit, deliberately not seeded: the smallest representable movement is
                // the whole point of the invariant, and any larger figure weakens it.
                final int n = 1000, opening = 100_000;
                led.seed("acct:a", opening);
                Tally t = new Tally();
                for (int i = 0; i < n; i++)
                    t.record(led.post(h.transfer("acct:a", "acct:b", 1, "penny-" + i)));
        ...
        new Invariant("INV-11", "Currencies cannot be mixed inside one transaction", Severity.MAJOR,
            ...
            (led, h) -> {
                ...
                PostResult r = led.post(new Transaction("fx-bad",
                        List.of(new Leg("acct:a", EntryType.DEBIT, amt, "USD"),
                                new Leg("acct:b", EntryType.CREDIT, amt, "EUR")),
                        h.txnId("fx-bad"), true, Map.of()));
        ...
        new Invariant("INV-12", "Reversal is compensation, never deletion", Severity.MAJOR,
            ...
            (led, h) -> {
                long amt = h.amount(1_000, 5_000);
                led.seed("acct:a", amt * 3);
                led.post(h.transfer("acct:a", "acct:b", amt, "orig"));
                List<String> original = fingerprint(led.journal());
                led.post(h.transfer("acct:b", "acct:a", amt, "comp-of-orig"));
                List<String> after = fingerprint(led.journal());

                // An ordered prefix, not containsAll: List.containsAll ignores multiplicity, so a
                // compensation that deleted one of two identical entries and appended others
                // would have passed. The entry count is not pinned either — a ledger that books a
                // fee reversal alongside the principal is correct, and failing it for that is a
                // finding about the invariant, not the ledger.
                if (after.size() <= original.size())
                    return Check.bad("journal held at " + after.size() + " entries after a "
                            + "compensating transaction, expected it to grow");
                for (int i = 0; i < original.size(); i++)
                    if (!after.get(i).equals(original.get(i)))
                        return Check.bad("compensation rewrote entry " + i + " of " + original.size()
                                + " — the original was modified rather than compensated");
                long net = led.balance("acct:b");
                return net == 0
                        ? Check.ok(original.size() + " original entries intact, "
                                + (after.size() - original.size()) + " compensating entries appended; "
                                + "net position " + money(net))
                        : Check.bad("net position after compensation is " + money(net) + ", expected "
                                + money(0));
            }),
        ...
        new Invariant("INV-13", "State rebuilds exactly from the event log", Severity.MAJOR,
            ...
            (led, h) -> {
                ...
                final long amt = h.amount(100, 600);
                for (int i = 0; i < accounts; i++) led.seed("acct:" + i, amt * 60);
                h.burst(300, i -> {
                    try {
                        led.post(h.transfer("acct:" + (i % accounts),
                                "acct:" + ((i + 2) % accounts), amt, "rb-" + i, true));
                    } catch (Exception e) { throw new RuntimeException(e); }
                });
                StringBuilder bad = new StringBuilder();
        ...
        new Invariant("INV-14", "Per-account ordering is monotonic", Severity.MINOR,
            ...
            (led, h) -> {
                final long amt = h.amount(50, 400);
                led.seed("acct:a", amt * 150);
                h.burst(150, i -> {
                    try {
                        led.post(h.transfer("acct:a", "acct:b", amt, "seq-" + i, true));
                    } catch (Exception e) { throw new RuntimeException(e); }
                });
    ...
    private static List<String> fingerprint(List<JournalEntry> j) {
        ...
        for (JournalEntry e : j)
            // The sequence is part of the identity: without it two identical legs fingerprint
            // the same, and a check that one was not swapped for the other cannot tell.
            out.add(e.sequence() + "|" + e.transactionId() + "|" + e.accountId()
                    + "|" + e.type() + "|" + e.amountSubunits());
        return out;
    }
     ...
     *
     * <p>Formatted by integer arithmetic on the digits. The obvious implementation —
     * {@code String.format("%.2f", subunits / 100.0)} — puts a {@code double} in a money
     * position inside the kit whose INV-10 fails a client's ledger for exactly that, and whose
     * M-05 mutant is named "amounts round-trip through double". It is also lossy above
     * 2^53 subunits.
     *
     * <p>Defaults to USD because every leg the suite posts is USD; use the two-argument form
     * anywhere a currency is actually in hand. JPY has no minor unit and KWD has three, so a
     * hard-coded two would report a loss of 498 yen as "4.98".
     */
    public static String money(long subunits) {
        return money(subunits, "USD");
    }

    public static String money(long subunits, String currencyCode) {
        String digits = Long.toString(subunits);          // correct even for Long.MIN_VALUE
        boolean negative = digits.startsWith("-");
        if (negative) digits = digits.substring(1);

        int exponent = fractionDigits(currencyCode);
        if (exponent == 0) return negative ? "-" + digits : digits;

        StringBuilder sb = new StringBuilder(digits);
        while (sb.length() <= exponent) sb.insert(0, '0');
        sb.insert(sb.length() - exponent, '.');
        return negative ? "-" + sb : sb.toString();
    }

    /** Unknown or non-decimal codes fall back to two, which is wrong less often than crashing. */
    private static int fractionDigits(String currencyCode) {
        try {
            int d = Currency.getInstance(currencyCode).getDefaultFractionDigits();
            return d < 0 ? 2 : d;
